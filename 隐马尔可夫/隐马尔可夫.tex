...
\maketitle
    ...
    \section{EM算法与Baum-Welch预测算法的不同}
        有没有想过李航老师书中为什么不写
        ...
        因为这是EM算法的通用表达式。这个表达式在Baum-Welch中需要变化一下:
        \[Q(\lambda,\overline{ \lambda})=\mathbb{E}_{P_{\overline{\lambda}}(\boldsymbol{z}_t\mid \boldsymbol{x})}\log P_\lambda(\boldsymbol{x},\boldsymbol{z}_t)\neq\mathbb{E}_{P_{\overline{\lambda}}(\boldsymbol{z}\mid \boldsymbol{x})}\log P_\lambda(\boldsymbol{x},\boldsymbol{z})\]

        不同就在于$\boldsymbol{z}$不再独立同分布，这也意味着不能再直接求解完全数据的似然概率
        \[P_\lambda(\boldsymbol{x},\boldsymbol{z})\]

        就算使用也是计算
        \[P_\lambda(\boldsymbol{x},\boldsymbol{z}_t)\]
